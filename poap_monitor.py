...
# ═══════════════════════════════════════════════════════════════════════════════
#  SECTION 1 — CONFIG
#  ↓↓↓  Edit the ACTIVE CONFIGURATION block below to switch mock / region  ↓↓↓
# ═══════════════════════════════════════════════════════════════════════════════

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# ── PRESETS (copy-paste the block you need into ACTIVE CONFIGURATION) ─────────
#
#  Mock1 EU:
#    POAP_FILE             = "LDC_R4.0_Mock1_EU_LoadPlan.xlsx"
#    POAP_SHEET            = "POAP-Mock1"
#    LOAD_PLAN_PATTERN     = "2. Load Plan-EU"
#    SUMMARY_PATTERN       = "3. Load Object Summary-EU"
#    EMAIL_SUBJECT_PREFIX  = "LDC R4.0 : MOCK1 : EU"
#    OUTLOOK_FOLDER        = "Archive/R4 Mock1 EU"
#
#  Mock2 EU:
#    POAP_FILE             = "LDC_R4.0_Mock2_EU_LoadPlan.xlsx"
#    POAP_SHEET            = "POAP-Mock1"
#    LOAD_PLAN_PATTERN     = "2. Load Plan-EU"
#    SUMMARY_PATTERN       = "3. Load Object Summary-EU"
#    EMAIL_SUBJECT_PREFIX  = "LDC R4.0 : MOCK2 : EU"
#    OUTLOOK_FOLDER        = "Archive/R4 Mock2 EU"
#
#  Mock2 LACAN:
#    POAP_FILE             = "LDC_R4.0_Mock2_LACAN_LoadPlan.xlsx"
#    POAP_SHEET            = "POAP-Mock2"
#    LOAD_PLAN_PATTERN     = "2. Load Plan-LACAN"
#    SUMMARY_PATTERN       = "3. Load Object Summary-LACAN"
#    EMAIL_SUBJECT_PREFIX  = "LDC R4.0 : MOCK2 : LACAN"
#    OUTLOOK_FOLDER        = "Archive/R4 Mock2 LACAN"
#
#  Mock3 EU:
#    POAP_FILE             = "LDC_R4.0_Mock3_EU_LoadPlan.xlsx"
#    POAP_SHEET            = "POAP-Mock3"
#    LOAD_PLAN_PATTERN     = "2. Load Plan-EU"
#    SUMMARY_PATTERN       = "3. Load Object Summary-EU"
#    EMAIL_SUBJECT_PREFIX  = "LDC R4.0 : MOCK3 : EU"
#    OUTLOOK_FOLDER        = "Archive/R4 Mock3 EU"
#
#  PROD EU:
#    POAP_FILE             = "LDC_R4.0_PROD_EU_LoadPlan.xlsx"
#    POAP_SHEET            = "POAP-PROD"
#    LOAD_PLAN_PATTERN     = "2. Load Plan-EU"
#    SUMMARY_PATTERN       = "3. Load Object Summary-EU"
#    EMAIL_SUBJECT_PREFIX  = "LDC R4.0 : PROD : EU"
#    OUTLOOK_FOLDER        = "Archive/PROD EU"
#
# ─────────────────────────────────────────────────────────────────────────────
...
POAP_SHEET           = "POAP-Mock1"
LOAD_PLAN_PATTERN    = "2. Load Plan-EU"          # substring to find the Load Plan sheet tab
SUMMARY_PATTERN      = "3. Load Object Summary-EU" # substring to find the Summary sheet tab

EMAIL_SUBJECT_PREFIX = "LDC R4.0 : MOCK2 : EU"   # emails must contain this string in subject
OUTLOOK_FOLDER       = ""                          # e.g. "Archive/R4 Mock2 EU"  — leave "" to scan entire mailbox
LOOKBACK_DAYS        = 90                          # days back to scan on first run

# ── General (rarely need to change) ──────────────────────────────────────────
MAX_EMAILS_PER_CHAIN = 10
POLL_SECONDS         = 60
DUE_HOURS            = 3
CACHE_OVERLAP_MINS   = 10
...
def load_objects():
    ...
    load_plan_sheet    = next((s for s in wb.sheetnames if LOAD_PLAN_PATTERN in s), None)

    load_summary_sheet = next((s for s in wb.sheetnames if SUMMARY_PATTERN in s), None)

    if not load_plan_sheet:

        print(f'  WARNING: No sheet matching "{LOAD_PLAN_PATTERN}" found in Excel.')

    if not load_summary_sheet:

        print(f'  WARNING: No sheet matching "{SUMMARY_PATTERN}" found in Excel.')
...
# ── Status table keywords ─────────────────────────────────────────────────────
# Maps lowercase table status text fragments → (activity, status)
_TABLE_STATUS_MAP = [
    ('pre-transformation completed',  ('Pre-T',     'Completed')),
    ('pre-transformation in progress',('Pre-T',     'In Progress')),
    ('pre-transformation started',    ('Pre-T',     'In Progress')),
    ('pre-transformation requested',  ('Pre-T',     'Requested')),
    ('transformation completed',      ('Transform', 'Completed')),
    ('transformation in progress',    ('Transform', 'In Progress')),
    ('transformation started',        ('Transform', 'In Progress')),
    ('transformation requested',      ('Transform', 'Requested')),
    ('pre-load completed',            ('Preload-Val','Completed')),
    ('pre-load in progress',          ('Preload-Val','In Progress')),
    ('preload completed',             ('Preload-Val','Completed')),
    ('preload in progress',           ('Preload-Val','In Progress')),
    ('load completed',                ('Load',      'Completed')),
    ('load in progress',              ('Load',      'In Progress')),
    ('load started',                  ('Load',      'In Progress')),
    ('load requested',                ('Load',      'Requested')),
    ('post-load completed',           ('Post',      'Completed')),
    ('post-load in progress',         ('Post',      'In Progress')),
    ('post load completed',           ('Post',      'Completed')),
    ('post load in progress',         ('Post',      'In Progress')),
    ('replication completed',         ('REPL',      'Completed')),
    ('replication in progress',       ('REPL',      'In Progress')),
    ('replication started',           ('REPL',      'In Progress')),
    ('extraction completed',          ('Extract',   'Completed')),
    ('extraction in progress',        ('Extract',   'In Progress')),
    ('not started',                   (None,        'Not Started')),
    ('completed',                     (None,        'Completed')),
    ('in progress',                   (None,        'In Progress')),
]

# Priority order for aggregating across multiple sites
_STATUS_PRIORITY = {
    'In Progress': 4, 'Requested': 3, 'Completed': 2,
    'Not Started': 1, 'Unknown': 0,
}


def _parse_status_table(body):
    """Extract rows from a status table in the email body.
    Returns list of dicts with keys: obj_id, site, dml_object, status, comments.
    Handles both plain-text (whitespace-separated) and HTML tables."""
    rows = []

    # ── HTML table ────────────────────────────────────────────────────────────
    html_tables = re.findall(r'<table[^>]*>.*?</table>', body, re.DOTALL | re.IGNORECASE)
    for table_html in html_tables:
        ...
        for idx, cells in enumerate(cells_per_row):
            upper = [c.upper() for c in cells]
            if 'OBJECT ID' in upper and 'STATUS' in upper:
                header_idx = idx
                col_map = {v: i for i, v in enumerate(upper)}
                break
        if header_idx is None:
            continue
        for cells in cells_per_row[header_idx + 1:]:
            if len(cells) < 2: continue
            rows.append({
                'obj_id':     cells[col_map.get('OBJECT ID', 0)].strip(),
                'site':       cells[col_map.get('SITE', 1)].strip()       if 'SITE' in col_map else '',
                'dml_object': cells[col_map.get('DML OBJECT', 2)].strip() if 'DML OBJECT' in col_map else '',
                'status':     cells[col_map.get('STATUS', 3)].strip()     if len(cells) > 3 else '',
                'comments':   cells[col_map.get('COMMENTS', 4)].strip()   if len(cells) > 4 else '',
            })
        if rows:
            return rows
    ...
    header_idx, col_starts = None, []
    for idx, line in enumerate(lines):
        upper = line.upper()
        if 'OBJECT ID' in upper and 'STATUS' in upper:
            header_idx = idx
            # Detect column start positions by locating header keywords
            for kw in ('OBJECT ID', 'SITE', 'DML OBJECT', 'STATUS', 'COMMENTS'):
                pos = upper.find(kw)
                if pos != -1:
                    col_starts.append((kw, pos))
            col_starts.sort(key=lambda x: x[1])
            break
    if header_idx is None or not col_starts:
        return []
    col_names = [c[0] for c in col_starts]
    col_pos   = [c[1] for c in col_starts]
    for line in lines[header_idx + 1:]:
        if not line or line.upper().startswith('OBJECT ID'): continue
        cells = []
        ...
        def _cell(name):
            return cells[col_names.index(name)] if name in col_names and col_names.index(name) < len(cells) else ''
        rows.append({
            'obj_id':     _cell('OBJECT ID'),
            'site':       _cell('SITE'),
            'dml_object': _cell('DML OBJECT'),
            'status':     _cell('STATUS'),
            'comments':   _cell('COMMENTS'),
        })
    return rows


def _map_table_status(status_text):
    """Map a status table cell value to (activity, status) using keyword rules."""
    lower = status_text.lower().strip()
    for fragment, result in _TABLE_STATUS_MAP:
        if fragment in lower:
            return result
    return (None, 'Unknown')


def _aggregate_table_rows(rows, obj_id):
    """Aggregate multiple site rows for one object into a single (activity, status)."""
    obj_rows = [r for r in rows if r.get('obj_id', '').upper() == obj_id.upper()]
    if not obj_rows:
        return None, None
    mapped = [_map_table_status(r['status']) for r in obj_rows]
    # Pick the activity from the most advanced/common row
    activities = [a for a, _ in mapped if a]
    activity   = max(set(activities), key=activities.count) if activities else None
    # Aggregate status: highest priority wins
    agg_status = max((s for _, s in mapped), key=lambda s: _STATUS_PRIORITY.get(s, 0))
    return activity, agg_status


def _detect_via_table_keywords(rows, obj_id, emails):
    """Build a detection result dict from parsed status table rows."""
    activity, agg_status = _aggregate_table_rows(rows, obj_id)
    if activity is None and agg_status in (None, 'Unknown'):
        return None
    email      = emails[0] if emails else {}
    dt         = email.get('date')
    stamp      = dt.strftime('%d-%b-%Y') if isinstance(dt, datetime) else str(dt or '')
    obj_rows   = [r for r in rows if r.get('obj_id', '').upper() == obj_id.upper()]
    sites_done = sum(1 for r in obj_rows if 'completed' in r.get('status','').lower())
    summary    = (f"[{stamp}] Table: {activity or 'Unknown'} — {agg_status} "
                  f"({sites_done}/{len(obj_rows)} sites completed)")
    return {
        'current_activity':  activity or 'Unknown',
        'activity_status':   agg_status,
        'summary':           summary,
        'blockers':          None,
...
def detect_current_activity(emails, obj_id=''):
    ...
    # ── Step 1: scan emails for a status table ────────────────────────────────
    table_rows   = []
    table_source = None
    for em in emails:
        rows = _parse_status_table(em.get('body', ''))
        if rows:
            table_rows   = rows
            table_source = em
            break
    ...
    # ── Step 2: table found ───────────────────────────────────────────────────
    if table_rows and obj_id:
        # 2a. Keyword result from table
        kw_result = _detect_via_table_keywords(table_rows, obj_id, emails)

        # 2b. AI result — send table content to Dify if enabled
        if LLM_ENABLED and kw_result:
            try:
                table_text = '\n'.join(
                    f"{r['obj_id']} | {r['site']} | {r['status']} | {r.get('comments','')}"
                    for r in table_rows if r.get('obj_id','').upper() == obj_id.upper()
                )
                ai_result = _detect_via_llm(
                    [{'subject': table_source.get('subject',''),
                      'sender':  table_source.get('sender',''),
                      'date':    table_source.get('date'),
                      'body':    f"STATUS TABLE FOR {obj_id}:\n{table_text}"}]
                )
                # Prefer AI result but keep keyword summary if AI gives Unknown
                if ai_result.get('current_activity') not in ('Unknown', None):
                    ai_result['summary'] = (
                        f"[Table+AI] {ai_result.get('summary','')}"
                    )
                    return ai_result
            except Exception as e:
                print(f'    LLM table detection failed ({e}) — using keyword table result.')

        if kw_result:
            return kw_result

    # ── Step 3: no table — fall back to email content detection ──────────────
    if LLM_ENABLED:
        try:
...
def run_once():
    ...
    if needs_llm:
        ...
        with ThreadPoolExecutor(max_workers=5) as pool:
            ...
            future_to_oid = {pool.submit(detect_current_activity, emails, oid): oid
